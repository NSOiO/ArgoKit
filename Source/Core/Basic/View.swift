...
public struct ArgoKitNodeAlias{
    weak var nodeAlias:ArgoKitNode?
    public func nodeView()->UIView?{
        return nodeAlias?.nodeView()
    }
}
...
extension View {
    ...
    /// - Parameter variable: The specified variable or property that is  assigned with the node.
    /// - Returns: self
    ///
    ///```
    ///    class DemoViewModel {
    ...
    public func nodeAlias(variable: inout ArgoKitNodeAlias?) -> Self {
        variable = ArgoKitNodeAlias(nodeAlias: self.node)
        return self
    }
}
