...
//action 相关
@property (nonatomic,strong)ArgokitThreadSafeDictionary<NSString *,ArgoKitNodeBlock> *actionMap;
...
// 计算当前node
//static void YGApplyLayoutToNodeHierarchy(ArgoKitNode *node,CGPoint offsetPoint = CGPointZero)
//{
//    const ArgoKitLayout *layout = node.layout;
//    YGNodeRef ygnode = layout.ygnode;
//    if (!ygnode || isnan(YGNodeLayoutGetWidth(ygnode)) || isnan(YGNodeLayoutGetHeight(ygnode))) {
...
//            ArgoKitNode *chiledNode = [node.childs objectAtIndex:i];
//            chiledNode.isReused = node.isReused;
//            chiledNode.parentNode = node.parentNode;
//            YGApplyLayoutToNodeHierarchy([node.childs objectAtIndex:i],{topLeft.x+offsetPoint.x,topLeft.y+offsetPoint.y});
//            chiledNode.isReused = NO;
//        }
//    }else{
//        const CGPoint origin = node.resetOrigin ? CGPointZero:node.frame.origin;
//        CGRect frame = (CGRect) {
//            .origin = {
//                .x = YGRoundPixelValue(topLeft.x + origin.x + offsetPoint.x),
//                .y = YGRoundPixelValue(topLeft.y + origin.y + offsetPoint.y),
//            },
//            .size = {
//                .width = YGRoundPixelValue(YGNodeLayoutGetWidth(ygnode)),
//                .height = YGRoundPixelValue( YGNodeLayoutGetHeight(ygnode)),
//            },
//        };
//        if (!CGRectEqualToRect(node.frame, frame)) {
//            node.frame = frame;
...
-(void)dealloc{
//    NSLog(@"ArgoKitNode dealloc %@",self);
...
- (void)_destroyProperties:(BOOL)dealloc {
    if (dealloc) {
        if (!self.completeDealloc) {
            ...
            [self iterationRemoveActionMap:self.childs isDealloc:dealloc];
...
- (void)iterationRemoveActionMap:(nullable NSArray<ArgoKitNode*> *)nodes isDealloc:(BOOL)dealloc{
    NSInteger nodeCount = nodes.count;
    for (int i = 0; i < nodeCount; i++) {
        ArgoKitNode *node = nodes[i];
        [node _destroyProperties:dealloc];
    }
}
...
- (void)_clearStrongRefrence{
    ...
    [self.viewAttributes argokit_removeAllObjects];
    self.viewAttributes = nil;
    self.lock = nil;
...
- (void)initContent{
    ...
    _bindProperties = [[ArgokitThreadSafeDictionary alloc] init];
    
    _actionMap = [[ArgokitThreadSafeDictionary alloc] init];
...
- (BOOL)virtualNode{
    return _isVirtualNode && !self.isRootNode;
}
...
- (void)resetAction:(ArgoKitNode *)reusedNode{
    if ((self.actionMap.count == 0
         || self.completeDealloc
         || self.completeDestroy)
         && reusedNode.actionMap.count > 0) {
        self.actionMap =  reusedNode.actionMap;
        [self.actionMap addEntriesFromDictionary:reusedNode.actionMap];
    }
    
    if ((self.nodeActions.count == 0
         || self.completeDealloc
         || self.completeDestroy)
         && reusedNode.nodeActions.count > 0) {
        self.nodeActions = reusedNode.nodeActions;
        NSArray<NodeAction *> *copyActions = [reusedNode.nodeActions mutableCopy];
...
- (nullable ArgoKitNode *)rootNode{
    if (!_rootNode) {
        ArgoKitNode *node = self;
        ArgoKitNode *temp = nil;
