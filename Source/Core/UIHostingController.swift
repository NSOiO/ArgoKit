...
//        }
    ...
    public override func sizeThatFits(_ size: CGSize) -> CGSize {
        ...
        if size.height <= 0 && !size.height.isNaN{
            height = .nan
        }
        let size  = rootView?.width(ArgoValue(width)).height(ArgoValue(height)).applyLayout(size: size)
        return size ?? CGSize.zero
    }
    
    /// Asks the view to calculate and return the size that best fits the specified width and height.
    /// - Parameters:
    ///   - width:  The width for which the view should calculate its best-fitting width.when width = .nan  A new width that fits the receiver’s subviews.
    ///   - height: The height for which the view should calculate its best-fitting height.when height = .nan  A new height that fits the receiver’s subviews.
    /// - Returns:  A new size that fits the receiver’s subviews.
