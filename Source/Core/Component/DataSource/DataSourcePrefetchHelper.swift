...
class DataSourcePrefetchHelper<D>{
    ...
    func runOperation(){
        ...
        prefetchModels_?.enumerateObjects({ (prefetchMode, index, pointer) in
            ...
            if let dataSourceHelper = innerOperation.dataSourceHelper,let indexPaths = innerOperation.indexPaths {
                for indexPath in indexPaths {
                    if let sourceData = dataSourceHelper.dataForRow(indexPath.row, at: indexPath.section),
                       let node = dataSourceHelper.nodeForData(sourceData){
                        dataSourceHelper.updateCellHeightForRow(node, maxWidth: innerOperation.width)
        ...
        if let removesModels = prefetchModels_ as? [Any] {
            self.prefetchModels.removeObjects(in:removesModels)
        }
