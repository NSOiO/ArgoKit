...
class ArgoKitScrollViewNode: ArgoKitArttibuteNode, UIScrollViewDelegate {
    ...
    weak var reusedNode:ArgoKitNode?
    var autoHeight:CGFloat = 0 {
        didSet{
            if let node = self.reusedNode {
                node.flexGrow(0)
                node.height(point:autoHeight)
            }
        }
    }
    ...
    func createContentNode() {
        ...
        nodeObserver.setCreateViewBlock { [weak self] view in
            if let strongSelf = self {
                ...
                if strongSelf.contentSize.equalTo(CGSize.zero){
                    strongSelf.contentSize =  CGSize(width: view.frame.width, height: CGFloat.nan)
                }
