...
public class List<D>: ScrollView  {
    ...
    override func createNode() {
        pNode = TableNode<D>(viewClass: TableView.self, type: Self.self)
    }
...
extension List{
    ...
    /// - Returns: Self
    @discardableResult
    public func estimatedRowHeight(_ value: @escaping @autoclosure () ->CGFloat) -> Self{
    ...
    public func relayoutList(value: @escaping @autoclosure () ->Bool) -> Self{
        return self.bindCallback({ [tableNode = self.tableNode] in
            let value = value()
            tableNode.setRelayoutList(value)
        }, forKey: #function)
    }
    
    public func removeNode(value: Bool) -> Self{
        tableNode.setRemoveNode(value)
        return self
    }
}
