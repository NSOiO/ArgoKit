...
private var DefaultItemSpaceValue: Float = 15
public class TabSegment: View {
    
    // MARK: - Default Value
    private var DefaultProgressBarWidth: Float = 20
    private var DefaultAnimationDuration: Float = 0.25
    private var DefaultScaleFromValue: Float = 1.0
    private var DefaultScaleToValue: Float = 1.3
    ...
    private let _contentView: HStack = {
        return HStack().justifyContent(.evenly).alignSelf(.stretch).alignItems(.stretch)
    }()
    
    private let _progressBar: Image = {
        return Image().backgroundColor(.black).width(20.0).height(5).cornerRadius(2)
    }()
    ...
    convenience public init(_ datas: Array<Any>, @ArgoKitListBuilder content: @escaping (Any) -> View) {
        self.init(datas, itemSpace: DefaultItemSpaceValue, content: content)
    }
    
    convenience public init(_ datas: Array<Any>, itemSpace: Float, @ArgoKitListBuilder content: @escaping (Any) -> View) {
        self.init(itemSpace)
        _container.node?.isVirtualNode = false
        _contentView.node?.row()
        _contentView.node?.isVirtualNode = false
...
extension TabSegment {
    ...
    public func progressBarWidth(_ value:Float) -> Self{
        DefaultProgressBarWidth = value
        _progressBar.width(ArgoValue(value))
        return self
    }
    
    public func progressBarHeight(_ value:Float) -> Self{
        _progressBar.height(ArgoValue(value))
        return self
    }

    public func progressBarColor(_ value:UIColor) -> Self{
        _progressBar.backgroundColor(value)
        return self
    }
    
    public func progressBarCornerRadius(_ value:CGFloat) -> Self{
        _progressBar.cornerRadius(value)
        return self
    }
    
    public func tabScaleFromValue(_ value:Float) -> Self{
        DefaultScaleFromValue = value
        return self
    }
    
    public func tabScaleToValue(_ value:Float) -> Self{
        DefaultScaleToValue = value
        return self
    }
}
    ...
    private func createSubviews(_ datas: Array<Any>, _ content: ((Any) -> View)?) {
        ...
        for (_, data) in datas.enumerated() {
            ...
            if let node = container.type.viewNodes()?.first {
                ...
                stack.node?.isVirtualNode = false
