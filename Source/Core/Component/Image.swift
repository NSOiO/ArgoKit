...
class ArgoKitImageNode: ArgoKitNode {
    ...
    public func image(url: URL?, placeholderImage: UIImage?) {
        self.url = url
        self.placeholderImage = placeholderImage
        if let imageView = self.nodeView() as? UIImageView {
            ArgoKitInstance.imageLoader()?.setImageForView(imageView, url: url, placeholder: placeholderImage, successed: nil, failure: nil)
        }
    }
    
    public func image(url: URL?, placeholder: String?) {
        let placeholderImage = (placeholder != nil && (placeholder?.count ?? 0) > 0) ? UIImage(named: placeholder!) : nil
        image(url: url, placeholderImage: placeholderImage)
    }
}
...
public struct Image : View {
    ...
    /// - Parameters:
    ///   - url: The url of a image.
    ///   - placeholderImage: The placeholder image when image is loading. You can set a image created by color as the placeholder image.
    public init(url: @escaping @autoclosure () -> URL?, placeholderImage: @escaping @autoclosure () -> UIImage?) {
        self.init(image: nil, highlightedImage: nil)
        self.bindCallback({ [self] in
            pNode.image(url: url(), placeholderImage: placeholderImage())
        }, forKey: #function)
    }
    ...
    ///   - urlString: The string represent a valid URL For a image.
    ///   - placeholderImage: The placeholder image when image is loading. You can set a image created by color as the placeholder image.
    public init(urlString: @escaping @autoclosure () -> String?, placeholderImage: @escaping @autoclosure () -> UIImage?) {
        self.init(image: nil, highlightedImage: nil)
        self.bindCallback({ [self] in
            var url: URL? = nil
            if let urlString = urlString() {
